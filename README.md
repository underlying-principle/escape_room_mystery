...
> **A time-loop murder mystery where your past self becomes the key to the case.**

A CrazyGames-oriented React + TypeScript + Vite HTML5 game foundation. Level 01 currently uses a lightweight 2D Canvas renderer and procedural Web Audio; the original Signal word-puzzle systems and map remain in the project for reuse. The player changes one letter at a time; every intermediate state must be a curated common English word. Twenty-two solver-validated levels across six worlds climb from basic transforms to lock-breaking wildcards, authored two-word reactions, and deep-difficulty finale worlds — each rendered over a procedural animated real-place backdrop with the level's playable word graph drawn as a living constellation.

## Current build status

Level 01 has been upgraded from the original simple escape loop into **The Room Remembers**: a deterministic time-loop investigation with persistent clue discovery, CCTV evidence, Badge 07 / Mara Voss suspect identification, incident timeline reconstruction, a hidden recorder, final authorization code `0317`, and a thriller-style corridor reveal. The existing visual atmosphere and ghost replay foundation are preserved.

The source tree is ready for the next build/install on a machine with the project dependencies available.
